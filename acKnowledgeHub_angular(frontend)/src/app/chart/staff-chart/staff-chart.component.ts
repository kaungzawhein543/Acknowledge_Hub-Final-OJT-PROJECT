...
export class StaffChartComponent implements AfterViewInit{
 ...
 createPieChart(): void {
  ...
  const showTotalStaff = false; // toggle this to true if you want to show total staff
  const labels = showTotalStaff ? ['Total Staff', 'Active Staff', 'Inactive Staff'] : ['Active Staff', 'Inactive Staff'];
  const data = showTotalStaff ? [
  ...
  const chart = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [{
        label: 'Staff Summary',
        data,
        backgroundColor: showTotalStaff ? [
          'rgba(255, 206, 86, 0.2)',
          'rgba(54, 162, 235, 0.2)',
          'rgba(255, 99, 132, 0.2)'
        ] : [
